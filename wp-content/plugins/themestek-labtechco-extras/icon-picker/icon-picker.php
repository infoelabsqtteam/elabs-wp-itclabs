...
/**
 *  Main list of icon library
 */
if( !function_exists('tste_labtechco_icon_libraries') ){
function tste_labtechco_icon_libraries(){
	
	/**
	There are three steps to add new icon library:
	
	1. Add library in this array
	2. Upload icon library files in /themestek-labtechco/extras/icon-picker/icon-libraries/ folder.
	3. Create new JS array file (iconset file) as /themestek-labtechco/extras/icon-picker/iconset/icon-LIBRARY.js file.
	
	Don't use - (dash) in folder or file name of icon library. Use _ (underscore) instead.
	...
	*/
	...
	$icon_library = array(
		...
		'fontawesome' => array( esc_attr__( 'FontAwesome', 'labtechco' ),		'fa fa-ok',					'/font-awesome/css/font-awesome.min.css' ),
		'themify'     => array( esc_attr__( 'Themify icons', 'labtechco' ),		'ti-location-pin',			'/themify-icons/themify-icons.css' ),
		'sgicon'      => array( esc_attr__( 'Skotroke Gap Icon', 'labtechco' ),	'sgicon sgicon-WorldWide',	'/stroke-gap-icons/style.css' ),
		'vc_linecons' => array( esc_attr__( 'Linecons', 'labtechco' ),			'li_star', 					'/vc-linecons/vc_linecons_icons.min.css' ),
		'ts_labtechco_business_icon' => array( esc_attr__( 'Lab Icon', 'labtechco' ),	'ts-labtechco-business-icon-nuclear', 					'/ts-lab-icons/font/flaticon.css' ),
	);
...
/**
 *  JS array files for icon libraries
 */
if( !function_exists('tste_labtechco_admin_iconpicker_scripts_styles') ){
function tste_labtechco_admin_iconpicker_scripts_styles() {
	
	$icon_libraries = tste_labtechco_icon_libraries();
	
	
	/** JS Files - Icon library array files **/
	foreach( $icon_libraries as $icon_id=>$icon_data ){
		wp_enqueue_script( 'iconset-'.$icon_id, THEMESTEK_LABTECHCO_URI . '/icon-picker/iconset/iconset-'.$icon_id.'.js', array( 'bootstrap' ) );
	}
	
	
	/** CSS Files - from Theme package **/
	foreach( $icon_libraries as $icon_id=>$icon_data ){
		$icon_css  = $icon_data[2];
		if( $icon_id != 'fontawesome'){
			wp_enqueue_style( $icon_id, THEMESTEK_LABTECHCO_URI.'/icon-picker/icon-libraries'.$icon_css ); // from plugin
		}
	}
	...
	wp_enqueue_script( 'bootstrap', THEMESTEK_LABTECHCO_URI . '/libraries/bootstrap/js/bootstrap.min.js', array( 'jquery' ) );
	
	// Bootstrap icon picker
	wp_enqueue_script( 'bootstrap-iconpicker', THEMESTEK_LABTECHCO_URI . '/libraries/bootstrap-iconpicker/js/bootstrap-iconpicker.js', array( 'bootstrap' ) );
	
	// Bootstrap icon picker - CSS
	wp_enqueue_style( 'bootstrap-iconpicker', THEMESTEK_LABTECHCO_URI . '/libraries/bootstrap-iconpicker/css/bootstrap-iconpicker.min.css' );
	
	// icon picker
	wp_enqueue_script( 'ts-icon-picker', THEMESTEK_LABTECHCO_URI . '/icon-picker/icon-picker.js', array( 'jquery' ) );
	
	
}
}
add_action( 'admin_enqueue_scripts', 'tste_labtechco_admin_iconpicker_scripts_styles' );
...
/**
 *  CSS libraries for frontend
 */
if( !function_exists('tste_labtechco_icon_libraries_css') ){
function tste_labtechco_icon_libraries_css() {
	
	$icon_libraries = tste_labtechco_icon_libraries();
	
	/** CSS Files - from Theme package **/
	foreach( $icon_libraries as $icon_id=>$icon_data ){
		if( $icon_id == 'fontawesome' ){ $icon_id = 'font-awesome'; }  // This is to make it compatible with VC font awesome library
		$icon_css  = $icon_data[2];
		wp_enqueue_style( $icon_id, THEMESTEK_LABTECHCO_URI.'/icon-picker/icon-libraries'.$icon_css ); // from plugin
	}
...
add_action( 'wp_enqueue_scripts', 'tste_labtechco_icon_libraries_css' );
...
/**
 *  Admin inline JS code.. specially added for iconpicker init function
 */
if( !function_exists('tste_labtechco_admin_iconpicker_library') ){
function tste_labtechco_admin_iconpicker_library(){
	?>
			...
			Iconpicker.ICONSET = {
				...
				<?php
					...
					foreach( $icon_libraries as $library_id=>$library_data ){
						if( $x != 1 ){ echo ','; }
						echo ' '.$library_id.'	: jQuery.iconset_'.$library_id.' || Iconpicker.ICONSET_EMPTY' . "\n";
						$x++;
					}
				...
				for (var i = 0; i < length; i++) {
					...
					for (var j = 0; j < op.cols; j++) {
						...
						if (pos < op.icons.length) {
							...
							var spaninner = $('<span class="ts-icon-tooltip"><i class="' + op.iconClass + ' ' + v + '"></i></span>');
							btn.append( spaninner );
...
add_action('admin_head', 'tste_labtechco_admin_iconpicker_library');
