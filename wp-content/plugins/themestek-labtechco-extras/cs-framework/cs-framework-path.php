...
if( ! function_exists( 'cs_get_path_locate' ) ) {
  function cs_get_path_locate() {
    ...
    $directory_uri  = str_replace( 'https://', '//', $directory_uri );
    $directory_uri  = str_replace( 'http://', '//', $directory_uri );
...
// CS_URI
$cs_uri = plugin_dir_url( __FILE__ );
if(substr($cs_uri, -1) == '/') {
    $cs_uri = substr($cs_uri, 0, -1);
}
if(substr($cs_uri, 0, 8) == 'https://') {
  $cs_uri = substr($cs_uri, 6);
}
if(substr($cs_uri, 0, 7) == 'http://') {
  $cs_uri = substr($cs_uri, 5);
}

// CS_DIR
$cs_dir = plugin_dir_path(__FILE__);
if(substr($cs_dir, -1) == '/') {
    $cs_dir = substr($cs_dir, 0, -1);
}


defined( 'CS_BASENAME' )  or  define( 'CS_BASENAME',  '/themestek-labtechco-extras/cs-framework' );
defined( 'CS_DIR' )       or  define( 'CS_DIR',       $cs_dir );
defined( 'CS_URI' )       or  define( 'CS_URI',       $cs_uri );
