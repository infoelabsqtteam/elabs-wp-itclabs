<?php
// [ts-custom-heading tag="h1" text="This is heading text"]
if( !function_exists('themestek_sc_custom_heading') ){
function themestek_sc_custom_heading( $atts, $content=NULL ){
	...
	if( function_exists('vc_map') ){
		...
		$options_list = ts_create_options_list($ts_sc_params_custom_heading);
		...
		$inline_style = '';
		$css_class    = 'ts-custom-heading';
		
		// converting vc value to array
		$font_container_data = ts_vc_parse_multi_attribute(
			$font_container,
			array(
				'tag'         => 'h2',
				'text_align'  => '',
		...
		$google_fonts_data   = ts_vc_parse_multi_attribute(
			$google_fonts,
			array(
				'font_family' => '',
				'font_style'  => '',
		...
		if( !empty($css_class) ){
			...
			$design_option_class = ts_vc_shortcode_custom_css_class( $css );
		...
		if( !empty($alldata['font_family']) ){
			...
			themestek_footer_google_fonts_array( $font_family , $font_weight );
		}
...
add_shortcode( 'ts-custom-heading', 'themestek_sc_custom_heading' );