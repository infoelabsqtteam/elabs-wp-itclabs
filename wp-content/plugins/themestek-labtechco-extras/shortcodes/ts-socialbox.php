...
function themestek_sc_social_box( $atts, $content=NULL ){
	...
	if( function_exists('vc_map') ){
		...
		$options_list = ts_create_options_list($ts_sc_params_socialbox);
		...
		$class[] = 'ts-socialbox-icon-size-' . $iconsize;
		$class[] = 'ts-socialbox-column-'.$column;
		...
		$return .= '<div class="ts-socialbox-links-wrapper row multi-columns-row">';
		...
		if( is_array($socialdata) && count($socialdata)>0 ){
			foreach( $socialdata as $data ){
				if( !empty($data->servicename) ){
					...
					$return .= '<div class="ts-socialbox-i-wrapper '.$columnclass.'">';
					$return .= '<a class="ts-socialbox-icon-link ts-socialbox-icon-link-'.$data->servicename.'" target="_blank" href="'.$servicelink.'" data-tooltip="'.$servicename.'"><i class="ts-labtechco-icon-'.$data->servicename.'"></i></a>';
					$return .= '</div>';
		...
		$return .= '</div><!-- .ts-socialbox-links-wrapper -->  ';
...
add_shortcode( 'ts-socialbox', 'themestek_sc_social_box' );