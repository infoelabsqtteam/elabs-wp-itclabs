...
if( !function_exists('themestek_sc_btn') ){
function themestek_sc_btn( $atts, $content=NULL ) {
	...
	if( function_exists('vc_map') ){
		...
		$options_list   = ts_create_options_list($ts_sc_params_btn);
		...
		// CSS classes in DIV
		$css_class   = array();
		$css_class[] = 'ts-vc_btn3-container';
		...
		if( !empty($css_animation) ){
			$css_animation = ts_getCSSAnimation( $css_animation );
		...
		$wrapper_classes = array(
			...
			ts_getExtraClass( $el_class ),
			ts_getCSSAnimation( $css_animation ),
			'ts-vc_btn3-' . $align,
		);
		
		
		//$button_classes[] = 'ts-vc_general';
		//$button_classes[] = 'ts-vc_btn3';
		
		$button_classes = array(
			'ts-vc_general',
			'ts-vc_btn3',
			'ts-vc_btn3-size-' . $size,
			'ts-vc_btn3-shape-' . $shape,
			'ts-vc_btn3-style-' . $style,
			'ts-vc_btn3-weight-' . $font_weight,
		);
		
		
		// Text style - adding outline class to set all text color
		if( $style=='text' ){
			//$button_classes[] = 'ts-vc_btn3-style-outline';
		}
		...
		$button_html = $title;
		if ( '' === trim( $title ) ) {
			$button_classes[] = 'ts-vc_btn3-o-empty';
			$button_html = '<span class="ts-vc_btn3-placeholder">&nbsp;</span>';
		}
		
		
		
		if ( 'true' === $button_block && 'inline' !== $align ) {
			$button_classes[] = 'ts-vc_btn3-block';
		}
		if ( 'true' === $add_icon ) {
			$button_classes[] = 'ts-vc_btn3-icon-' . $i_align;
			ts_vc_icon_element_fonts_enqueue( $i_type );
			...
			if ( $icon_wrapper==true ) {
				$icon_html = '<i class="ts-vc_btn3-icon"><span class="ts-vc_btn3-icon-inner ' . esc_attr( $icon_class ) . '"></span></i>';
			} else {
				$icon_html = '<i class="ts-vc_btn3-icon ' . esc_attr( $icon_class ) . '"></i>';
			}
		...
		if ( 'custom' === $style ) {
			if ( $custom_background ) {
				$styles[] = ts_vc_get_css_color( 'background-color', $custom_background );
			}

			if ( $custom_text ) {
				$styles[] = ts_vc_get_css_color( 'color', $custom_text );
			}

			if ( ! $custom_background && ! $custom_text ) {
				$button_classes[] = 'ts-vc_btn3-color-grey';
			}
		} elseif ( 'outline-custom' === $style ) {
			if ( $outline_custom_color ) {
				$styles[] = ts_vc_get_css_color( 'border-color', $outline_custom_color );
				$styles[] = ts_vc_get_css_color( 'color', $outline_custom_color );
			...
			if ( ! $outline_custom_color && ! $outline_custom_hover_background && ! $outline_custom_hover_text ) {
				$button_classes[] = 'ts-vc_btn3-color-inverse';

				foreach ( $button_classes as $k => $v ) {
					if ( 'ts-vc_btn3-style-outline-custom' === $v ) {
			...
			$uid = uniqid();
			$return .= '<style type="text/css">.ts-vc_btn3-style-' . $style . '.ts-vc_btn-gradient-btn-' . $uid . ':hover{' . implode( ';',
					$gradient_css_hover ) . ';' . '}</style>';
			$return .= '<style type="text/css">.ts-vc_btn3-style-' . $style . '.ts-vc_btn-gradient-btn-' . $uid . '{' . implode( ';',
					$gradient_css ) . ';' . '}</style>';
			$button_classes[] = 'ts-vc_btn-gradient-btn-' . $uid;
			$attributes[] = 'data-vc-gradient-1="' . $gradient_color_1 . '"';
			$attributes[] = 'data-vc-gradient-2="' . $gradient_color_2 . '"';
		} else {
			$button_classes[] = 'ts-vc_btn3-color-' . $color;
		}
...
add_shortcode( 'ts-btn', 'themestek_sc_btn' );