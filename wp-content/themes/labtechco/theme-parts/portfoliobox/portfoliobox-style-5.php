<article class="themestek-box themestek-box-portfolio ts-portfoliobox-style-5 themestek-box-view-overlay ts-hover-style-3">
	<div class="themestek-post-item">
		<?php echo themestek_featured_image('themestek-img-600x720'); ?>
		<div class="themestek-box-content themestek-overlay">
			<div class="themestek-box-content-inner">			  
				<div class="themestek-pf-box-title">
					<div class="themestek-box-category"><?php echo themestek_portfolio_category(true); ?></div>
					<h3><a title="<?php the_title(); ?>" href="<?php echo get_permalink(); ?>"><?php the_title(); ?></a></h3>					
				</div>
            </div>
		</div>
	</div>
</article>