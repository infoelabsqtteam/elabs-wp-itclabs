...
$portfolioList = array();
if( taxonomy_exists('ts-portfolio-category') ){
	$portfolioList_data = get_posts( array(
		'post_type'        => 'ts-portfolio',
		'numberposts'      => -1,
	) );
	$portfolioList      = array();
	foreach($portfolioList_data as $post_array){
		$portfolioList[ esc_attr($post_array->post_title) ] = esc_attr($post_array->ID);
	}
}
...
$allParams = array_merge(
		...
		array(
			...
			array(
				...
				"value"       => array(
					...
					esc_attr__("50 pixel spacing", "labtechco")               => "50px",
				),
				"std"  => "",
				'group'		  => esc_attr__( 'Box Style', 'labtechco' ),
			)
		...
		array( 
			...
			array(
				"type"        => "checkbox",
				"heading"     => sprintf( esc_attr__('Selected %1$s (Posts) Only', "labtechco"), $pf_type_title ),
				"description" => sprintf( esc_attr__('Select %1$s (Posts) that will appear.', 'labtechco'), $pf_type_title ),
...
vc_map( array(
	"name"     => sprintf( esc_attr__("PBM %s Box",'labtechco'), $pf_type_title_singular ),
