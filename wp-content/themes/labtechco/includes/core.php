	...
	if( !empty($post_type) ){
		...
		if( !empty( $social_services ) && is_array( $social_services ) ){
			foreach( $social_services as $social ){
				...
				<a href="javascript:void(0)" onClick="TSSocialWindow=window.open(\''. esc_url($link) .'\',\'TSSocialWindow\',width=600,height=100); return false;" title="' . esc_attr__( 'Share', 'labtechco' ) . '"><i class="ts-labtechco-icon-'. sanitize_html_class($social) .'"></i></a>
				</li>';
...
function ts_shared_social_list(){
	...
	$sociallist = array(
		...
		'tiktok'		=> esc_attr__('Tiktok', 'labtechco' ),
...
function ts_carousel_data_html( $allVar ){
	$return = '';
	if( !empty($allVar['settings']) ){
		$allVar = $allVar['settings'];
	}
			...
			switch( $meta_id ){
				...
				case 'date':
					...
					if ( in_array( get_post_type(), array( 'post', 'attachment' ) ) ) {
						...
						$return .= sprintf( '<span class="ts-meta-line posted-on"><span class="screen-reader-text ts-hide">%1$s </span><a href="%2$s" rel="bookmark" title="'. esc_attr__( 'Posted on', 'labtechco' ) .'">%3$s</a></span>',
				...
				case 'author':
					...
					if ( 'post' === get_post_type() ) {
						// preparing link
						$author	= '<a title="'.get_the_author().'" class="url fn n" href="' . esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) . '">'.get_the_author().'</a>';
			...
			switch( $type ){
				...
				case 'phone':
					$return = '<div class="ts-team-list-title ts-skincolor">'. esc_attr__('Phone','labtechco') .'</div>
						<div class="ts-team-list-value"><a title="' . $return . '" href="tel:' . $return . '">' . $return . '</a></div>';
					break;

				case 'email':
					$return = '<div class="ts-team-list-title ts-skincolor">'. esc_attr__('Email','labtechco') .'</div>
						<div class="ts-team-list-value"><a title="' . $return . '" href="mailto:' . $return . '">' . $return . '</a></div>';
					break;
				...
				case 'website':
					...
					$return = '<div class="ts-team-list-title ts-skincolor">'. esc_attr__('Website','labtechco') .'</div>
						<div class="ts-team-list-value"><a title="'. esc_attr__('Website','labtechco') .'" target="_blank" href="' . esc_url($return_link) . '">' . $return . '</a></div>';
					break;
		...
		foreach( $team_extra_details_lines as $key=>$val ){
			...
			if( !empty($post_meta['ts_team_info']['team_extra_details_line_'.$key]) ){
				...
				if ( $val['data']=='url' ){
					$return_link = $value;
					if( substr($return_link, 0, 3)=='www' ){
						$return_link = 'http://'.$value;
					}
					$return .= '<li class="ts-team-details-line ts-team-extra-details-line-' . $key . '">
					<div class="ts-team-list-title ts-skincolor">' . $title . '</div>
					<div class="ts-team-list-value"><a title="' . $title . '" target="_blank" href="' . esc_url($return_link) . '">' . $value . '</a></div>
					</li>';
				} else if( $val['data']=='email' ){
					$return .= '<li class="ts-team-details-line ts-team-extra-details-line-' . $key . '">
					<div class="ts-team-list-title ts-skincolor">' . $title . '</div>
					<div class="ts-team-list-value"><a title="' . $title . '" href="mailto:' . $value . '">' . $value . '</a></div>
					</li>';
				} else if( $val['data']=='phone' ){
					$return .= '<li class="ts-team-details-line ts-team-extra-details-line-' . $key . '">
					<div class="ts-team-list-title ts-skincolor">' . $title . '</div>
					<div class="ts-team-list-value"><a title="' . $title . '" href="tel:' . $value . '">' . $value . '</a></div>
					</li>';
				} else{
					$return .= '<li class="ts-team-details-line ts-team-extra-details-line-' . $key . '">
					<div class="ts-team-list-title ts-skincolor">' . $title . '</div>
...
function themestek_team_member_single_category_list( $post_id='' ){
	...
	if( is_array($categories_list) && count($categories_list)>0 ){
		...
		foreach( $categories_list as $category ){
			if( $x != 1 ){ $return .= '&nbsp; / &nbsp;'; } $x++;
			$return .= '<a title="' . esc_attr($category->name) . '" href="' . get_term_link( $category->term_id ) . '">' . esc_attr($category->name) . '</a>';
...
function themestek_portfoliobox_likes(){
	...
	$likeIconClass   = ( isset($_COOKIE["themestek_likes_".get_the_ID()]) ) ? 'tsicon-fa-heart' : 'tsicon-fa-heart-o' ;
		...
		$like = '<div class="themestek-portfolio-likes-wrapper">
					<a title="'.esc_attr__( 'Like', 'labtechco' ).'" class="themestek-portfolio-likes ' . $likeActiveClass . '" href="#" id="pid-' . get_the_ID() . '">
...
function themestek_portfolio_shortdesc( $post_id='' ){
	...
	if( is_singular() ){
		...
		if( !empty($short_desc['ts_pf_shortdesc']) ){
			$short_desc = wpautop( $short_desc['ts_pf_shortdesc'] );
...
function themestek_portfolio_related(){
	...
	if( $portfolio_show_related===true ){
		...
		// Enqueue portfolio related style 
		$min = TS_MIN;
		if( is_child_theme() ){
			$min = '';
		}
		
		wp_enqueue_style( 'ts-portfoliobox-style-'.$boxstyle, get_template_directory_uri() . '/css/portfoliobox/portfoliobox-'.$boxstyle.TS_MIN.'.css' );
...
// If no featured content found
	...
	// Adding class for general style
...
function themestek_get_featured_media( $post_id='', $size='full', $imgonly=false ){
	...
	if( !empty($post_id) ){
		...
		// If service
		if( $post_type=='ts-service' ){

			// get post meta
			$post_meta  = get_post_meta( $post_id, 'themestek_service_featured', true );
...
function themestek_featured_link($code=''){
	...
	if( get_post_format() == 'link' ){
		...
		if( !empty($link_url) ){
			$content .= '<h3 class="ts-format-link-title"><a title="' . get_the_title() . '" href="' . esc_url($link_url) . '">' . get_the_title() . '</a></h3>';
			$content .= '<span class="ts-format-link-url"><a title="' . get_the_title() . ' href="' . esc_url($link_url) . '">' . esc_url($link_url) . '</a></span>';
		} else {
...
function themestek_featured_quote($code=''){
	...
	if( get_post_format() == 'quote' ){
		...
		$source_content = '';
		if( !empty($source_url) ){
			$link_start = '<a title="'. esc_attr__( 'Link', 'labtechco' ) .'" href="' . $source_url . '">';
			$link_end   = '</a>';
		}
...
function themestek_get_boxes( $cpt='blog', $vars=array() ){
		...
		while ( $posts->have_posts() ) {
			...
			if( !empty($vars['sortable']) && $vars['sortable']=='yes' ){
				...
				foreach( $post_terms as $term ){
					$sortable_category[ $term->name ] = 'ts-term-'.$term->term_id;
				}
			}
			...
			if( $cpt == 'client' ){
				...
				if( $add_link == 'yes' && themestek_client_single_link()!='' ){
					$client_wrap_start .= '<a title="'. get_the_title()  .'" class="ts-client-logo-link" href="' . themestek_client_single_link() . '">';
	...
	if( !empty($return) ){
		...
		if( !empty($sortable_category) && is_array($sortable_category) && count($sortable_category)>0 ){
			$sortable_category_html .= '<li class="ts-sortable-link ts-sortable-all-link"><a title="'.$allword.'" class="selected" href="javascript:void(0);" data-filter="*"> ' . $allword . ' </a></li>';
			foreach($sortable_category as $key=>$val){
				$sortable_category_html .= '<li class="ts-sortable-link"><a title="'.$key.'" href="javascript:void(0);" data-filter=".' . $val . '">' . $key . '</a></li>';
			}
...
function themestek_get_taxonomy_from_cpt(){
	...
	} else if( get_post_type() == 'ts-service' ){
		$return = 'ts-service-category';
		...
		switch( $for ){
			case 'team':
				...
				$return = array(
					...
					array(
						'label'	=> esc_attr__('Team Member Box - Style 5','labtechco'),
						'value'	=> 'style-5',
						'thumb'	=> get_template_directory_uri() . '/includes/images/team-style-5.jpg',
					),
					array(
						'label'	=> esc_attr__('Team Member Box - Style 6','labtechco'),
						'value'	=> 'style-6',
						'thumb'	=> get_template_directory_uri() . '/includes/images/team-style-6.jpg',
					),
					array(
						'label'	=> esc_attr__('Team Member Box - Style 7','labtechco'),
						'value'	=> 'style-7',
						'thumb'	=> get_template_directory_uri() . '/includes/images/team-style-7.jpg',
					),
					array(
						'label'	=> esc_attr__('Team Member Box - Style 8','labtechco'),
						'value'	=> 'style-8',
						'thumb'	=> get_template_directory_uri() . '/includes/images/team-style-8.jpg',
					),
			...
			case 'portfolio':
				...
				$return = array(
					...
					array(
						'label'	=> esc_attr__('Portfolio Box - Style 4','labtechco'),
						'value'	=> 'style-4',
						'thumb'	=> get_template_directory_uri() . '/includes/images/portfolio-style-4.jpg',
					),
					array(
						'label'	=> esc_attr__('Portfolio Box - Style 5','labtechco'),
						'value'	=> 'style-5',
						'thumb'	=> get_template_directory_uri() . '/includes/images/portfolio-style-5.jpg',
					),
					array(
						'label'	=> esc_attr__('Portfolio Box - Style 6','labtechco'),
						'value'	=> 'style-6',
						'thumb'	=> get_template_directory_uri() . '/includes/images/portfolio-style-6.jpg',
					),
					array(
						'label'	=> esc_attr__('Portfolio Box - Style 7','labtechco'),
						'value'	=> 'style-7',
						'thumb'	=> get_template_directory_uri() . '/includes/images/portfolio-style-7.jpg',
					),
					array(
						'label'	=> esc_attr__('Portfolio Box - Style 8','labtechco'),
						'value'	=> 'style-8',
						'thumb'	=> get_template_directory_uri() . '/includes/images/portfolio-style-8.jpg',
					),
					array(
						'label'	=> esc_attr__('Portfolio Box - Style 9','labtechco'),
						'value'	=> 'style-9',
						'thumb'	=> get_template_directory_uri() . '/includes/images/portfolio-style-9.jpg',
					),
				);
				break;
			...
			case 'service':
				...
				$return = array(
					...
					array(
						...
						'value'	=> 'style-3',
						'thumb'	=> get_template_directory_uri() . '/includes/images/servicebox-style-3.jpg',
					),
					array(
						'label'	=> esc_attr__('Service Box - Style 4','labtechco'),
						'value'	=> 'style-4',
						'thumb'	=> get_template_directory_uri() . '/includes/images/servicebox-style-4.jpg',
					),
					array(
						'label'	=> esc_attr__('Service Box - Style 5','labtechco'),
						'value'	=> 'style-5',
						'thumb'	=> get_template_directory_uri() . '/includes/images/servicebox-style-5.jpg',
					),
					array(
						'label'	=> esc_attr__('Service Box - Style 6','labtechco'),
						'value'	=> 'style-6',
						'thumb'	=> get_template_directory_uri() . '/includes/images/servicebox-style-6.jpg',
					),
					array(
						'label'	=> esc_attr__('Service Box - Style 7','labtechco'),
						'value'	=> 'style-7',
						'thumb'	=> get_template_directory_uri() . '/includes/images/servicebox-style-7.jpg',
					),
					array(
						'label'	=> esc_attr__('Service Box - Style 8','labtechco'),
						'value'	=> 'style-8',
						'thumb'	=> get_template_directory_uri() . '/includes/images/servicebox-style-8.jpg',
					),
					array(
						'label'	=> esc_attr__('Service Box - Style 9','labtechco'),
						'value'	=> 'style-9',
						'thumb'	=> get_template_directory_uri() . '/includes/images/servicebox-style-9.jpg',
					),
					array(
						'label'	=> esc_attr__('Service Box - Style 10','labtechco'),
						'value'	=> 'style-10',
						'thumb'	=> get_template_directory_uri() . '/includes/images/servicebox-style-10.jpg',
					),
					array(
						'label'	=> esc_attr__('Service Box - Style 11','labtechco'),
						'value'	=> 'style-11',
						'thumb'	=> get_template_directory_uri() . '/includes/images/servicebox-style-11.jpg',
					),
					array(
						'label'	=> esc_attr__('Service Box - Style 12','labtechco'),
						'value'	=> 'style-12',
						'thumb'	=> get_template_directory_uri() . '/includes/images/servicebox-style-12.jpg',
					),
					array(
						'label'	=> esc_attr__('Service Box - Style 13','labtechco'),
						'value'	=> 'style-13',
						'thumb'	=> get_template_directory_uri() . '/includes/images/servicebox-style-13.jpg',
					),
					array(
						'label'	=> esc_attr__('Service Box - Style 14','labtechco'),
						'value'	=> 'style-14',
						'thumb'	=> get_template_directory_uri() . '/includes/images/servicebox-style-14.jpg',
					),
				);
				break;
			...
			case 'testimonial':
				...
				$return = array(
					...
					array(
						'label'	=> esc_attr__('Testimonial Box - Style 4','labtechco'),
						'value'	=> 'style-4',
						'thumb'	=> get_template_directory_uri() . '/includes/images/testimonial-style-4.jpg',
					),
					array(
						'label'	=> esc_attr__('Testimonial Box - Style 5','labtechco'),
						'value'	=> 'style-5',
						'thumb'	=> get_template_directory_uri() . '/includes/images/testimonial-style-5.jpg',
					),
					array(
						'label'	=> esc_attr__('Testimonial Box - Style 6','labtechco'),
						'value'	=> 'style-6',
						'thumb'	=> get_template_directory_uri() . '/includes/images/testimonial-style-6.jpg',
					),
					array(
						'label'	=> esc_attr__('Testimonial Box - Style 7','labtechco'),
						'value'	=> 'style-7',
						'thumb'	=> get_template_directory_uri() . '/includes/images/testimonial-style-7.jpg',
					),
					array(
						'label'	=> esc_attr__('Testimonial Box - Style 8','labtechco'),
						'value'	=> 'style-8',
						'thumb'	=> get_template_directory_uri() . '/includes/images/testimonial-style-8.jpg',
					),
					array(
						'label'	=> esc_attr__('Testimonial Box - Style 9','labtechco'),
						'value'	=> 'style-9',
						'thumb'	=> get_template_directory_uri() . '/includes/images/testimonial-style-9.jpg',
					),
					array(
						'label'	=> esc_attr__('Testimonial Box - Style 10','labtechco'),
						'value'	=> 'style-10',
						'thumb'	=> get_template_directory_uri() . '/includes/images/testimonial-style-10.jpg',
					),
			...
			case 'blog':
				...
				$return = array(
					...
					array(
						'label'	=> esc_attr__('Blog Box - Style 3','labtechco'),
						'value'	=> 'style-3',
						'thumb'	=> get_template_directory_uri() . '/includes/images/blogbox-style-3.jpg',
					),
					array(
						'label'	=> esc_attr__('Blog Box - Style 4','labtechco'),
						'value'	=> 'style-4',
						'thumb'	=> get_template_directory_uri() . '/includes/images/blogbox-style-4.jpg',
					),
					array(
						'label'	=> esc_attr__('Blog Box - Style 5','labtechco'),
						'value'	=> 'style-5',
						'thumb'	=> get_template_directory_uri() . '/includes/images/blogbox-style-5.jpg',
					),
					array(
						'label'	=> esc_attr__('Blog Box - Style 6','labtechco'),
						'value'	=> 'style-6',
						'thumb'	=> get_template_directory_uri() . '/includes/images/blogbox-style-6.jpg',
					),
					array(
						'label'	=> esc_attr__('Blog Box - Style 7','labtechco'),
						'value'	=> 'style-7',
						'thumb'	=> get_template_directory_uri() . '/includes/images/blogbox-style-7.jpg',
					),
					array(
						'label'	=> esc_attr__('Blog Box - Style 8','labtechco'),
						'value'	=> 'style-8',
						'thumb'	=> get_template_directory_uri() . '/includes/images/blogbox-style-8.jpg',
					),
					array(
						'label'	=> esc_attr__('Blog Box - Style 9','labtechco'),
						'value'	=> 'style-9',
						'thumb'	=> get_template_directory_uri() . '/includes/images/blogbox-style-9.jpg',
					),
					array(
						'label'	=> esc_attr__('Blog Box - Style 10','labtechco'),
						'value'	=> 'style-10',
						'thumb'	=> get_template_directory_uri() . '/includes/images/blogbox-style-10.jpg',
					),
			...
			case 'iconheadingbox':
				...
				$return = array(
					...
					array(
						'label'	=> esc_attr__('Service Box - Style 7','labtechco'),
						'value'	=> 'style-7',
						'thumb'	=> get_template_directory_uri() . '/includes/images/sbox-style-7.jpg',
					),
					array(
						'label'	=> esc_attr__('Service Box - Style 8','labtechco'),
						'value'	=> 'style-8',
						'thumb'	=> get_template_directory_uri() . '/includes/images/sbox-style-8.jpg',
					),
					array(
						'label'	=> esc_attr__('Service Box - Style 9','labtechco'),
						'value'	=> 'style-9',
						'thumb'	=> get_template_directory_uri() . '/includes/images/sbox-style-9.jpg',
					),
					array(
						'label'	=> esc_attr__('Service Box - Style 10','labtechco'),
						'value'	=> 'style-10',
						'thumb'	=> get_template_directory_uri() . '/includes/images/sbox-style-10.jpg',
					),
					array(
						'label'	=> esc_attr__('Service Box - Style 11','labtechco'),
						'value'	=> 'style-11',
						'thumb'	=> get_template_directory_uri() . '/includes/images/sbox-style-11.jpg',
					),
					array(
						'label'	=> esc_attr__('Service Box - Style 12','labtechco'),
						'value'	=> 'style-12',
						'thumb'	=> get_template_directory_uri() . '/includes/images/sbox-style-12.jpg',
					),
					array(
						'label'	=> esc_attr__('Service Box - Style 13','labtechco'),
						'value'	=> 'style-13',
						'thumb'	=> get_template_directory_uri() . '/includes/images/sbox-style-13.jpg',
					),
					array(
						'label'	=> esc_attr__('Service Box - Style 14','labtechco'),
						'value'	=> 'style-14',
						'thumb'	=> get_template_directory_uri() . '/includes/images/sbox-style-14.jpg',
					),
					array(
						'label'	=> esc_attr__('Service Box - Style 15','labtechco'),
						'value'	=> 'style-15',
						'thumb'	=> get_template_directory_uri() . '/includes/images/sbox-style-15.jpg',
					),
					array(
						'label'	=> esc_attr__('Service Box - Style 16','labtechco'),
						'value'	=> 'style-16',
						'thumb'	=> get_template_directory_uri() . '/includes/images/sbox-style-16.jpg',
					),
					array(
						'label'	=> esc_attr__('Service Box - Style 17','labtechco'),
						'value'	=> 'style-17',
						'thumb'	=> get_template_directory_uri() . '/includes/images/sbox-style-17.jpg',
					),
					array(
						'label'	=> esc_attr__('Service Box - Style 18','labtechco'),
						'value'	=> 'style-18',
						'thumb'	=> get_template_directory_uri() . '/includes/images/sbox-style-18.jpg',
					),
					array(
						'label'	=> esc_attr__('Service Box - Style 19','labtechco'),
						'value'	=> 'style-19',
						'thumb'	=> get_template_directory_uri() . '/includes/images/sbox-style-19.jpg',
					),
					array(
						'label'	=> esc_attr__('Service Box - Style 20','labtechco'),
						'value'	=> 'style-20',
						'thumb'	=> get_template_directory_uri() . '/includes/images/sbox-style-20.jpg',
					),
					array(
						'label'	=> esc_attr__('Service Box - Style 21','labtechco'),
						'value'	=> 'style-21',
						'thumb'	=> get_template_directory_uri() . '/includes/images/sbox-style-21.jpg',
					),
				);
				break;
			...
			case 'fidbox':
				...
				$return = array(
					...
					array(
						'label'	=> esc_attr__('Facts in Digits Box - Style 4','labtechco'),
						'value'	=> 'style-4',
						'thumb'	=> get_template_directory_uri() . '/includes/images/fidbox-style-4.jpg',
					),
					array(
						'label'	=> esc_attr__('Facts in Digits Box - Style 5','labtechco'),
						'value'	=> 'style-5',
						'thumb'	=> get_template_directory_uri() . '/includes/images/fidbox-style-5.jpg',
					),
					array(
						'label'	=> esc_attr__('Facts in Digits Box - Style 6','labtechco'),
						'value'	=> 'style-6',
						'thumb'	=> get_template_directory_uri() . '/includes/images/fidbox-style-6.jpg',
					),
					array(
						'label'	=> esc_attr__('Facts in Digits Box - Style 7','labtechco'),
						'value'	=> 'style-7',
						'thumb'	=> get_template_directory_uri() . '/includes/images/fidbox-style-7.jpg',
					),
					array(
						'label'	=> esc_attr__('Facts in Digits Box - Style 8','labtechco'),
						'value'	=> 'style-8',
						'thumb'	=> get_template_directory_uri() . '/includes/images/fidbox-style-8.jpg',
					),
					array(
						'label'	=> esc_attr__('Facts in Digits Box - Style 9','labtechco'),
						'value'	=> 'style-9',
						'thumb'	=> get_template_directory_uri() . '/includes/images/fidbox-style-9.jpg',
					),
					array(
						'label'	=> esc_attr__('Facts in Digits Box - Style 10','labtechco'),
						'value'	=> 'style-10',
						'thumb'	=> get_template_directory_uri() . '/includes/images/fidbox-style-10.jpg',
					),
					array(
						'label'	=> esc_attr__('Facts in Digits Box - Style 11','labtechco'),
						'value'	=> 'style-11',
						'thumb'	=> get_template_directory_uri() . '/includes/images/fidbox-style-11.jpg',
					),
					array(
						'label'	=> esc_attr__('Facts in Digits Box - Style 12','labtechco'),
						'value'	=> 'style-12',
						'thumb'	=> get_template_directory_uri() . '/includes/images/fidbox-style-12.jpg',
					),
					array(
						'label'	=> esc_attr__('Facts in Digits Box - Style 13','labtechco'),
						'value'	=> 'style-13',
						'thumb'	=> get_template_directory_uri() . '/includes/images/fidbox-style-13.jpg',
					),
					array(
						'label'	=> esc_attr__('Facts in Digits Box - Style 14','labtechco'),
						'value'	=> 'style-14',
						'thumb'	=> get_template_directory_uri() . '/includes/images/fidbox-style-14.jpg',
					),
				);
				break;
			...
			$return = array(
				array(
					'label'	=> esc_attr__('Action Box - Style 1','labtechco'),
					'value'	=> 'style-1',
					'thumb'	=> get_template_directory_uri() . '/includes/images/actionbox-style-1.jpg',
				),
			...
			case 'clients':
				...
				$return = array(
					...
					array(
						'label'	=> esc_attr__('Clients Box - Style 3','labtechco'),
						'value'	=> 'style-3',
						'thumb'	=> get_template_directory_uri() . '/includes/images/clientbox-style-3.jpg',
					),
				);
				break;
...
function themestek_box_title() {
	...
	if( 'link' != get_post_format() && 'quote' != get_post_format() ){
		$return = '<div class="themestek-box-title"><h4><a title="' . get_the_title() . '" href="' . get_permalink() . '">' . get_the_title() . '</a></h4></div>';
	}
...
function themestek_blogbox_readmore(){
	...
	if( get_post_format()!='quote' && get_post_format()!='link' ) {
		...
		$return .= '<a title="'.$readMore_text.'" href="'.get_permalink().'">'.$readMore_text.'</a>';
		$return .= '</div>';
	}
 ...
 * Print Read More link
 *
 * Create your own themestek_blogbox_readmore_classic() to override in a child theme.
 *
 * @since LabtechCO 1.0
 *
 * @return void
 */
if ( !function_exists( 'themestek_blogbox_readmore_classic' ) ){
function themestek_blogbox_readmore_classic(){

	$read_more_text = '<a title="'. esc_attr__( 'Link', 'labtechco' ) .'" href="'.get_permalink().'">'.themestek_get_option('blog_readmore_text').'</a>';
	...
	if( has_excerpt() && $blog_classic_excerpt_enable=='yes' ){
		the_excerpt();
		$return .= '<a title="'.$read_more_text.'" href="'.get_permalink().'">'.$read_more_html.'</a>';

	} else {
		$readMore_text = themestek_blogbox_readmore_text();
		$return .= '<a title="'.$readMore_text.'" href="'.get_permalink().'">'.$readMore_text.'</a>';
 ...
 * Single Service icon
 */
if ( !function_exists( 'themestek_service_icon' ) ){
function themestek_service_icon() {
	$val = get_post_meta( get_the_ID(), '_themestek_metabox_service_icon', true );
	if( !empty($val) ){
		if( !empty($val['themestek_custom_icon_enabled']) && $val['themestek_custom_icon_enabled'] == true ){
			$attachment = wp_get_attachment_image_src( $val['custom_service_icon'], 'full' );
			$alt_text = get_post_meta($val['custom_service_icon'] , '_wp_attachment_image_alt', true);
			$service_icon = '<img src="'.$attachment[0].'" alt="'. $alt_text .'"/>';
		} else{
			$service_icon = themestek_create_icon_from_data( $val['service_icon'], true );
		}
		echo themestek_wp_kses($service_icon);
	}
}
}
...
function themestek_box_wrapper( $position='start', $cptname='blog', $vars=array() ){
	...
	if( $position=='start' ){
		...
		$classArray[] = 'themestek-boxes-element-'.$cptname.'-'.$boxstyle;
...
function themestek_get_query_args( $cptname='blog', $vars=array() ){
	...
	$selected_posts = (!empty($vars['selected_posts'])) ? $vars['selected_posts'] : '' ;
	...
	if( count($vars)>0 ){
		...
		if( in_array( $cptname, $valid_post_types ) ){
			...
			// Creating array for multiple category
			if( strpos($selected_posts, ',') !== false ) {
...
function themestek_blogbox_single_meta( $option='date', $args=array() ) {
	...
	switch($option){
		...
		case 'user' :
			...
			$return = sprintf( '<span class="author vcard"><a title="'. get_the_author() .'" class="url fn n" href="%1$s">%2$s</a></span>',
...
function themestek_titlebar_inline_style(){
	...
	if( $post_id ){
		...
		if( !empty($titlebar_bg_featured_img) && is_array($titlebar_bg_featured_img) ){
			if( is_singular() && in_array( get_post_type() , $titlebar_bg_featured_img ) && !empty($featured_img) ){
...
function themestek_topbar_content(){
	...
	if( themestek_fbar_show()==true ){
		...
		<span class="themestek-fbar-btn ' . themestek_sanitize_html_classes(themestek_fbar_btn_classes()) . '">
			<a title="' . esc_attr__('Open', 'labtechco') . '" href="#" class="themestek-fbar-btn-link">
	...
	if( !empty($topbar_right_text) ){
		$topbar_right_text = '<div class="ts-table-cell ts-align-right">'. $topbar_right_text .'</div>';
	}
	...
	// print data
	echo do_shortcode(themestek_wp_kses($return));
...
function themestek_topbar_inline_style(){
	...
	if( $topbar_bg_color=='custom' ){
		$return .= '.themestek-pre-header-wrapper{background-color:'. $topbar_bg_custom_color .';}';
	}

	// Text Color CSS code
	if( $topbar_text_color=='custom' ){
		$return .= '.themestek-pre-header-wrapper, .themestek-pre-header-wrapper a{color:var(--tste-labtechco-topbar-text-custom-color );}';
	}
...
function themestek_site_logo( $logo_type = '' ){
	...
	$return .= '<div class="headerlogo themestek-logotype-'. sanitize_html_class($logotype) .' ts-stickylogo-'. sanitize_html_class($stickyLogoClass) .'">';
		...
		$return .= '<span class="logo-link-text">' . get_bloginfo( 'name' ) . ' - ' . get_bloginfo( 'description' ) . '</span>';
...
function themestek_header_links(){
	...
	if( $header_search==true ){
		$class   = '';
		$return .= '<span class="ts-header-icon ts-header-search-link"><a title="'. esc_attr__( 'Search', 'labtechco' ) .'" href="#"><i class="ts-labtechco-icon-search-2"></i></a></span>';
	}
	...
	if( function_exists('is_woocommerce') && $wc_header_icon==true ){
		...
		$return  .= '<span class="ts-header-icon ts-header-wc-cart-link"><a title="' . esc_attr($count) . '" href="' . $cart_url . '"><i class="ts-labtechco-icon-cart"></i><span class="number-cart">' . esc_attr($count) . '</span></a></span>';
	}
...
function themestek_header_class( $extra_class='' ){
	...
	if( $header_text_abovemenu=='' && '4'==$headerstyle ){
...
function themestek_header_style_class( $echo=false ){
	...
	if ($headerstyle == '5' || $headerstyle=='3'){
	...
	if ( (strpos( themestek_get_headerstyle(), 'rtl') !== false) || ((is_rtl()) && (in_array($curr_headerstyle, array('1','3','classic-box-overlay','2','infostack-overlay')))) ) {
...
function themestek_fbar_btn(){
	$return = '<!-- Open/close button -->
			<span class="themestek-fbar-btn ' . themestek_sanitize_html_classes(themestek_fbar_btn_classes()) . '">
				<a title="' . esc_attr__('Open', 'labtechco') . '" href="javascript:void(0)" class="themestek-fbar-btn-link">
					' . themestek_fbar_open_icon() . '
					' . themestek_fbar_close_icon() . '
					<span class="ts-hide">' . esc_attr__('Open', 'labtechco') . '</span>
				</a>
...
function themestek_box_team_social_links( $multicolor_icon = false ){
	...
	if( !empty($data) && is_array($data) && count($data)>0 ){
		...
		foreach($data as $social ){
			...
			$return .= '
				<li>
					<a title="'. $social_name .'" href="'. $social['social_icons_list_link'] .'" target="_blank" class="'.$a_color_class.'">
...
function themestek_testimonial_title(){
	...
	$return .= ( !empty($clienturl) ) ? '<h3 class="themestek-author-name"><a title="' . get_the_title() . '" href="'.esc_url($clienturl).'" target="_blank">'.get_the_title().'</a></h3>' : '<h3 class="themestek-author-name">'.get_the_title().'</h3>' ;
...
function themestek_header_button( $args=array() ){
	...
	if( !empty($btn_text) && !empty($btn_link) && function_exists('themestek_url_encode') ){
		if( function_exists('vc_map') ){
			...
			$btn_sc		= '<a href="' . esc_url($btn_link) . '">' . esc_attr($btn_text) . '</a>';
		}
...
if( !function_exists('themestek_headerstyle3_button') ){
function themestek_headerstyle3_button( $args=array() ){
	$return = '';
	$header_btn3 = themestek_get_option('third_sty_header_btn');
	$btn_text3 = ( !empty($header_btn3['third_sty_header_btn_text']) ) ? $header_btn3['third_sty_header_btn_text'] : '' ;
	$btn_link3 = ( !empty($header_btn3['third_sty_header_btn_link']) ) ? $header_btn3['third_sty_header_btn_link'] : '' ;
	if( !empty($btn_text3) && !empty($btn_link3) && function_exists('themestek_url_encode') ){
		if( function_exists('vc_map') ){
			$btn_sc		= '[ts-btn title="' . esc_attr($btn_text3) . '" shape="square" style="flat" color="skincolor" size="md" link="url:' . themestek_url_encode($btn_link3) . '|||"]';
		} else {
			$btn_sc		= '<a href="' . esc_url($btn_link3) . '">' . esc_attr($btn_text3) . '</a>';
		}
		$return		= '<div class="ts-header-text-area ts-header-button-w">'.do_shortcode( $btn_sc ).'</div>';
	}
	echo  themestek_wp_kses($return);
...
if( !function_exists('ts_header_button_second') ){
	function ts_header_button_second( $args = array() ){
		$btn_text  = themestek_get_option('header-btn2-text');
		$btn_url   = themestek_get_option('header-btn2-url');
		if(!empty($btn_text) && !empty($btn_url) ){
			?>
			<div class="ts-header-button">			
				<a  href="<?php echo esc_url($btn_url); ?>">
					<?php if(!empty($btn_text)) : ?><span class="ts-header-button-text-1"><?php echo esc_html($btn_text); ?></span><?php endif; ?>
				</a>
			</div>
			<?php
		}
...
if( !function_exists('ts_header_button_third') ){
function ts_header_button_third( $args = array() ){
	$btn3_text1 = themestek_get_option('header-btn3-text1');
	$btn3_text2 = themestek_get_option('header-btn3-text2');
	$btn3_url = themestek_get_option('header-btn3-url');
	if(!empty($btn3_text1) && !empty($btn3_text2) && !empty($btn3_url) ){
		?>
		<div class="ts-header-button3">
			<a href="<?php echo esc_url($btn3_url); ?>">
				<?php if(!empty($btn3_text1)) : ?><span class="ts-header-button-text-1"><?php echo esc_html($btn3_text1); ?></span><?php endif; ?>
				<?php if(!empty($btn3_text2)) : ?><span class="ts-header-button-text-2"><?php echo esc_html($btn3_text2); ?></span><?php endif; ?>
			</a>
		</div>
	<?php
 	}
...
function themestek_get_clientboxes( $vars = array() ){
	...
	foreach( $list_of_clients as $key => $val ){
		...
		if( !empty( $client_logo ) ){
			$return .= themestek_column_div( 'start', $column );
			$return .= '<a title="'.esc_attr( $client_name ).'" href="'.esc_url( $client_website ).'" '.$linktarget.' data-tooltip="'.esc_attr( $client_name ).'">';
			$return .= $client_logo;
			$return .= '</a>';
			$return .= themestek_column_div( 'end', $column );
		} else {
...
function themestek_event_description(){
	...
	if( has_excerpt() ){
		...
		$return .= '<div class="themestek-post-readmore"><a title="'. $readMore .'" href="'.get_permalink().'">'.$readMore.'</a></div>';
	} else {
		global $more;
		$more = 0;
...
function themestek_comment_row_template($comment, $args, $depth){
    ...
    <<?php echo esc_attr($tag); ?> <?php comment_class( empty( $args['has_children'] ) ? '' : 'parent' ) ?> id="comment-<?php comment_ID() ?>">
		...
		<div class="comment-meta commentsetadata">
			...
			<a title="<?php echo esc_attr__( 'Comment', 'labtechco' ); ?>" href="<?php echo htmlspecialchars( get_comment_link( $comment->comment_ID ) ); ?>">
			<?php
...
function themestek_author_social_links(){
	...
	foreach( $all_socials as $social_class => $social_link ){
		if( !empty($social_link) ){
			$return .= '<li><a title="'. $social_class .'" href="'. $social_link .'" target="_blank"><i class="ts-labtechco-icon-'. $social_class .'"></i><span class="ts-hide">'. ucwords($social_class) .'</span></a></li>';
		}
	}
...
function themestek_search_results_box_title( $post_type='post' ){
	...
	if( empty($_GET['post_type']) ){
		$small_link = '<small><a title="'.esc_attr__('View more','labtechco').'" href="'. esc_url(get_home_url()).'?s='.get_search_query().'&post_type=' . $post_type . '" class="label label-default">'.esc_attr__('View more','labtechco').'</a></small>';
	}
...
function themestek_nothing_found_message(){
	$return       = '';
	$results_text = '';
	$results_not_found_text = themestek_get_option('searchnoresult');
	...
	if( !empty($cpt_obj) ){
		foreach( $cpt_obj as $cpt ){
			if( !in_array( $cpt->name, array( 'attachment', 'ts-client', 'ts-testimonial' ) ) ){  // Exclude Media from search
				...
				$results_text  .= ( $total_results > 0 ) ? '  <li><a title="'. sprintf( esc_attr__('%1$s found in %2$s','labtechco'), $total_results, $cpt->labels->name ).'" href="' . esc_url($link) . '"> <i class="ts-labtechco-icon-angle-right"></i> ' . sprintf( esc_attr__('%1$s found in %2$s','labtechco'), $total_results, '<strong>'.$cpt->labels->name.'</strong>' ) . '</a></li>' : '' ;
	...
	if( !empty($results_text) ){
		...
		$return .= '</div>';
	} else if ( !empty($results_not_found_text) ) {
		$return = '<p>'.$results_not_found_text.'</p>';
	}
	else {
...
function themestek_search_form(){
	...
	if( !empty($cpt_obj) ){
		...
		foreach( $cpt_obj as $cpt ){
			if( !in_array( $cpt->name, array( 'attachment', 'ts-client', 'ts-testimonial', 'elementor_library' ) ) ){  // Exclude Media from search
				...
				if( $total_results > 0 ){
					...
					$cpt_tab .= '<li class="ts-search-form-tab ' . esc_attr($current_class) . '"><a title="'. $cpt->name  .'" href="#" data-cpt="' . $cpt->name . '"> ' . $cpt->label . $results_text . '</a></li>';
				}
...
function themestek_recent_posts( $post='' ){
	...
	$return .= '<li class="ts-recent-post-list-li">';
		if( has_post_thumbnail() ){
			$return .= '<a title="' . get_the_title() . '" href="' . get_permalink() . '">' . get_the_post_thumbnail( get_the_ID(), 'thumbnail') . '</a>';
		}
		$return .= '<a title="' . get_the_title() . '" href="' . get_permalink() . '">' . get_the_title() . '</a>';
...
function themestek_wp_kses( $string, $allowed_html_type='' ){
	...
	$allowed_html = array(
		...
		'div' => array(
			...
			'data-loop'			=> array(),
			'data-ts-autoplay'		=> array(),
			'data-ts-centermode'		=> array(),
			'data-ts-nav'			=> array(),
			'data-ts-dots'			=> array(),
			'data-ts-autoplayspeed'=> array(),
			'data-ts-loop'=> array(),
			'data-ts-pauseonfocus'=> array(),
			'data-ts-pauseonhover'=> array(),
			'data-ts-slidestoscroll'=> array(),
			'data-ts-effecttype'=> array(),
			'data-text' => array(),
		),
		'span'   => array(
			'class'  => array(),
...
function labtechco_blogstyle2_meta( $metafor="blogbox" ) {
	...
	// meta details
	if( !empty($return) ){
		$return = '<div class="entry-meta ts-entry-meta ts-entry-meta-' . $metafor . '">' . $return . '</div>' ;
	}
...
function labtechco_posttime_meta( $metafor="blogbox" ) {
	...
	$date_format  = themestek_get_option( $metafor . '_meta_dateformat' );
		...
		if ( in_array( get_post_type(), array( 'post', 'attachment' ) ) ) {
			...
			$return .= sprintf( '<span class="ts-meta-line posted-on"><span class="screen-reader-text ts-hide">%1$s </span><a title="%3$s" href="%2$s" rel="bookmark">%3$s</a></span>',
				esc_attr_x( 'Posted on', 'Used before publish date.', 'labtechco' ),
				esc_url( get_permalink() ),
				$time_string
			);
...
function themestek_get_socialbar_links( $tooltip_position='top' , $tooltip='yes' ){
	...
	if( !empty($labtechco_theme_options['social_icons_list']) ){
		foreach( $labtechco_theme_options['social_icons_list'] as $socialicon ){
			...
			$return .= '<li class="ts-social-' . $social_id . ' ts-socialbox-i-wrapper"><a title="'.$social_name.'" class="ts-socialbox-icon-link ts-socialbox-icon-link-' . $social_id . ' ' . sanitize_html_class($class) . '" target="_blank" href="' . $href . '"><span class="frame-hover"></span><i class="ts-labtechco-icon-' . $social_id . '"></i><span class="social_name">'.$social_name.'</span></a></li>' . "\n";
		}
	}
	...
	foreach( $socialArray as $key=>$value ){
		...
		if( $key == 'rss' ){
			if( isset($labtechco_theme_options['rss']) && $labtechco_theme_options['rss']=='1' ){
				$return .= '<li class="'.$key.'"><a title="'.$value[1].'" target="_blank" href="'.get_bloginfo('rss2_url').'" data-tooltip="'.$value[1].'"><i class="ts-social-icon-'.$value[0].'"></i></a></li>';
			}
		} else {
			if( isset($labtechco_theme_options[$key]) && trim($labtechco_theme_options[$key])!='' ){
				$return .= '<li class="'.$key.' "><a title="'.$value[1].'" target="_blank" href="'.esc_url($labtechco_theme_options[$key]).'" data-tooltip="'.$value[1].'"><i class="ts-social-icon-'.$value[0].'"></i></a></li>';
			}
		}
	}
...
if( !function_exists('tste_get_all_option_array') ) {
function tste_get_all_option_array(){
	$return = array();
	if( file_exists(get_template_directory() . '/cs-framework-override/config/framework-options.php') ){
		include( get_template_directory() . '/cs-framework-override/config/framework-options.php' );
	}
	...
	foreach( $ts_framework_options as $fields ){
		...
		if( isset($fields['sections']) && is_array($fields['sections']) && count($fields['sections'])>0 ){
			foreach( $fields['sections'] as $section ){
				foreach( $section['fields'] as $field ){
					if( !empty($field['id']) ){
						$settings            = str_replace( '-', '_', $field['id'] );
						$settings            = str_replace( '-', '_', $settings );
						$settings            = str_replace( '-', '_', $settings );
						$settings            = str_replace( '-', '_', $settings );
						$settings            = str_replace( '-', '_', $settings );
						$return[ $settings ] = themestek_get_option( $field['id'] );
					}
				}
			}
		}
...
if ( !function_exists( 'tste_hex2rgb' ) ){
function tste_hex2rgb($color, $opacity='1'){
	$default = 'rgb(0,0,0)';
	if (empty($color))
		return $default;
	if ($color[0] == '#')
...
if ( !function_exists( 'tste_hex2rgb_code' ) ){
function tste_hex2rgb_code($color){
	$default = 'rgb(0,0,0)';
	if (empty($color))
		return $default;	
	if ($color[0] == '#')
...
if ( !function_exists( 'tste_add_ordinal_number_suffix' ) ){
function tste_add_ordinal_number_suffix($num) {
...
if( !function_exists('ts_get_elements') ){
function ts_get_elements( $data = array() ){
	$el_array = array();
	if( !empty($data) && is_array($data) ){
		foreach( $data as $s1 ){

			if( isset($s1['elType']) && $s1['elType']!='section' && $s1['elType']!='column' && substr( $s1['widgetType'], 0 , 3 ) == 'ts_' ){
					$style = ( isset($s1['settings']['boxstyle']) && !empty($s1['settings']['boxstyle']) ) ? trim($s1['settings']['boxstyle']) : '1' ;
			...
			if( isset($s1['elements']) && !empty($s1['elements']) ){
				foreach($s1['elements'] as $s2){
					if( isset($s2['elType']) && $s2['elType']!='section' && $s2['elType']!='column' && substr( $s2['widgetType'], 0 , 3 ) == 'ts_' ){
						$style = ( isset($s2['settings']['boxstyle']) && !empty($s2['settings']['boxstyle']) ) ? trim($s2['settings']['boxstyle']) : '1' ;
						$style = str_replace('style-', '', $style);
					...
					if( isset($s2['elements']) && !empty($s2['elements']) ){
						foreach($s2['elements'] as $s3){
							if( isset($s3['elType']) && $s3['elType']!='section' && $s3['elType']!='column' && substr( $s3['widgetType'], 0 , 3 ) == 'ts_' ){
								$style = ( isset($s3['settings']['boxstyle']) && !empty($s3['settings']['boxstyle']) ) ? trim($s3['settings']['boxstyle']) : '1' ;
								$style = str_replace('style-', '', $style);
							...
							if( isset($s3['elements']) && !empty($s3['elements']) ){
								foreach($s3['elements'] as $s4){
									if( isset($s4['elType']) && $s4['elType']!='section' && $s4['elType']!='column' && substr( $s4['widgetType'], 0 , 3 ) == 'ts_' ){
										$style = ( isset($s4['settings']['boxstyle']) && !empty($s4['settings']['boxstyle']) ) ? trim($s4['settings']['boxstyle']) : '1' ;
										$style = str_replace('style-', '', $style);
									...
									if( isset($s4['elements']) && !empty($s4['elements']) ){
										foreach($s4['elements'] as $s5){
											if( isset($s5['elType']) && $s5['elType']!='section' && $s5['elType']!='column' && substr( $s5['widgetType'], 0 , 3 ) == 'ts_' ){
												$style = ( isset($s5['settings']['boxstyle']) && !empty($s5['settings']['boxstyle']) ) ? trim($s5['settings']['boxstyle']) : '1' ;
												$style = str_replace('style-', '', $style);
											...
											if( isset($s5['elements']) && !empty($s5['elements']) ){
												foreach($s5['elements'] as $s6){
													if( isset($s6['elType']) && $s6['elType']!='section' && $s6['elType']!='column' && substr( $s6['widgetType'], 0 , 3 ) == 'ts_' ){
														$style = ( isset($s6['settings']['boxstyle']) && !empty($s6['settings']['boxstyle']) ) ? trim($s6['settings']['boxstyle']) : '1' ;
														$style = str_replace('style-', '', $style);
...
if( !function_exists('ts_element_block_container') ){
function ts_element_block_container( $settings = array( 'position' => 'start', 'column' => 'three', 'cpt' => 'blog', ) ){
		$return = '';
		...
		if( $settings['position']=='start' ){
			switch( $settings['column'] ){
				case 'one':
					$return = '<div class="ts-box-col-wrapper ts-'.$cpt.'-ele col-lg-12 col-sm-12 col-md-12 col-xs-12">';
					break;
				case 'two':
					$return = '<div class="ts-box-col-wrapper ts-'.$cpt.'-ele col-lg-6 col-sm-6 col-md-6 col-xs-12">';
					break;
				case 'three':
					$return = '<div class="ts-box-col-wrapper ts-'.$cpt.'-ele col-lg-4 col-sm-6 col-md-4 col-xs-12">';
					break;
				case 'four':
					$return = '<div class="ts-box-col-wrapper ts-'.$cpt.'-ele col-lg-3 col-sm-6 col-md-3 col-xs-12">';
					break;
				case 'five':
					$return = '<div class="ts-box-col-wrapper ts-'.$cpt.'-ele col-lg-20percent col-sm-4 col-md-4 col-xs-12">';
					break;
				case 'six':
					$return = '<div class="ts-box-col-wrapper ts-'.$cpt.'-ele col-lg-2 col-sm-4 col-md-4 col-xs-12">';
					break;
			}
